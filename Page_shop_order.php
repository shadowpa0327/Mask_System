        ...
        <script>
            ...
            $.fn.serializeObject = function(){
                ...
                for(let i =delete_idx.length-1;i>=0;i--){
                    console.log("delete:",delete_idx[i])
                ...
                if(total==1){
                    ...
                    for(let cnt = 0;cnt < data_length ; cnt++ ){
                        ...
                        $('#show').append(`<tr>
                                    ...
                                    <td><button type = "submit" value="Finish" class="btn btn-success btn-sm submit-button" name="${cnt}">V</button>
                    ...
                    onPageClick: function (event, page) {
                        ...
                        for(let cnt = (page-1)*per_page , idx = 0;  idx<per_page && cnt < data_length ; idx++,cnt++ ){
                            ...
                            //console.log(cnt,' ',jsArray[cnt]['EndTime']);
                            $('#show').append(`<tr>
                                    <td><input type="checkbox" class="order_check_box"  id="checkbox${cnt}" name="multiselect[]" value =${cnt}  style="display:none"></input></td>
                                    <td>${jsArray[cnt]['OID']}</td>
                                    <td>${jsArray[cnt]['status']}</td>
            ...
            $(document).ready(function() {
                    ...
                    $(document).on('click','.submit-button',function(e){
                        ...
                        if($(this).attr('value')=="Finish")update_mode = "Finished";
                ...
                <div class="card-body">
                    <form class="login" onsubmit="">
                        <div class="form-group row">
                            <label for="amount" class="col-sm-2 col-form-label">Store_name</label>
                            <select name="shop">
                                    <?php
                                         ...
                                         $sql = "((SELECT store_name FROM Clerk where username = :uname) union (SELECT store_name FROM Store where shop_holder = :uname))" ;
                                         $pre_query = $conn->prepare($sql);
                                         $pre_query->execute(array("uname"=>$uname ));
                                         $result = $pre_query ->fetchall();
                                         //echo($pre_query->rowCount());
                                         foreach($result as $val){
                                             //echo($val['store_name']);
                                             echo('<option value="'.$val['store_name'].'">'.$val['store_name'].'</option>');
                                         }
                                    ?>
                            </select>
                        </div>
            ...
            <div class="card bg-light mb-6" style="max-width:60rem;">
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between">
                        ...
                        <div>
                            <button type = "submit" value="Cancel" class="btn btn-danger btn-sm submit-button  order_check_box" style="display:none" name="multi_cancel" id="multi_button_cancel">Cancel</button>
                            <button type = "submit" value="Finish" class="btn btn-success btn-sm submit-button  order_check_box " style="display:none" name="multi_select" id="multi_button_select">Finish</button>
                        </div>
                    </div>
                </div>
